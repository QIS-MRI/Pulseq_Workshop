...
% k_traj = continuous k-space trajectory from gradients
% ktraj_adc = sampled trajectory points where ADC reads data
...
% map ADC samples into the correct grid position in k-space
...
%kx = ADC samples along readout kx
% data_coils_last = [kx, coils, acquisitions]
data_coils_last=reshape(data_coils_last, [size(data_coils_last,1)*size(data_coils_last,2), nCoils]);
% reorder into k-space grid
...
% used for noise handling to improve SNR, combines only magnitude 
figure;
if nCoils>1
...
%if there are two echoes, it computes a phase difference image.
if size(images,3)==2
